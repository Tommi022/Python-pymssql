...
class WrapperDB:
    ...
    def elencoPost(self, as_dict = False):
        ...
        try:
            ...
            sql = "SELECT Id, Autore, Testo, [Like] FROM FALC_FB_Post ORDER BY [Like] DESC"
            cur.execute(sql)
            lista = cur.fetchall()
        except:
    ...
    def inserisciPost(self, parametri):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "INSERT INTO FALC_FB_Post (Autore, Testo) VALUES (%s , %s)"
    ...
    def daiLikeAPost(self, id, is_like = True):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "UPDATE FALC_FB_Post SET [Like] = "
            if is_like == True: 
                sql += "[Like] + 1 "
            else:
                sql += "[Like] - 1 "
    ...
    def eliminaPost(self, id):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "DELETE FALC_FB_Post WHERE id = %d"
            cursore.execute(sql, id)
            c.commit()
    ...
    def aggiungiCommenti(self, parametri):
        try:
            c = self.connetti()
            cursore = c.cursor()
            sql = "INSERT INTO FALC_FB_Commenti (idPost, Autore, Testo) VALUES (%d, %s , %s)"
            cursore.execute(sql, parametri)
            c.commit()
    ...
    def elencoCommenti(self, as_dict = False):
        ...
        try:
            cur = conn.cursor(as_dict = as_dict)
            sql = "SELECT Id, Autore, Testo, [Like] FROM FALC_FB_Commenti ORDER BY [Like] DESC"
            cur.execute(sql)
            lista = cur.fetchall()
        except:
            err = "Houston abbiamo un problema..."
    ...
    def singoloCommento(self, id):
        #restituisce un singolo commento
        ret = {}
        conn = self.connetti()
        try:
            cursore = conn.cursor(as_dict = True)
